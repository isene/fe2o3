...
| isotopes | <https://github.com/isene/isotopes> | Chart of the nuclides (3,386 isotopes from the IAEA table, decay modes, half-lives, decay chains, the whole chart in real pixels where the terminal shows images) |
| particles | <https://github.com/isene/particles> | Standard Model explorer (17 fundamental particles + proton/neutron with PDG values, and a 3D zoom atom → nucleus → nucleon → quark, real pixels where the terminal shows images) |
...
| fractal | <https://github.com/isene/fractal> | Chaos and fractals in real pixels, braille where the terminal has none (Mandelbrot, Julia, logistic map, Lorenz, Hénon) |
...
| `fe2o3-starmap` | `starmap` | <https://github.com/isene/starmap> | The naked-eye sky, real pixels through glow or braille, star colours from each star's temperature: 9,096 stars (Yale BSC + Hipparcos distances), constellation figures, horizon + hemisphere projections, interactive star picker |
...
| `fe2o3-funkey` | `funkey` | <https://github.com/isene/funkey> | Game engine for the terminal (battery rules waived for games). A pixel Frame shown as half-block cells or as real pixels through kitty graphics, Input with exact key holds where the terminal reports releases, a fixed-step run loop, Sprite (rows or PNG), Tilemap and Body physics, two fonts, particles, Audio (a mixer with WAV, Doom and synthesized samples and tunes), a software 3D Raster, a textured Scene painted on every core (textures with holes and mip levels, a light per corner, sphere culling, fog, a sky), a WAD loader and a sector/BSP renderer, and a script runner for terminal-free tests and films. Examples: `climb`, `jumpman` (Jumpman Junior in spirit, five levels), `invaders` (Space Invaders in spirit), `drive` (packages against the clock, in 3D), `soar` (a flight over ray-cast fractal mountains), `castle` (a walk to a textured castle) and `doom` (the full game on any Doom WAD). `funkeys` picks a game. The crate version is the engine's; each game has its own version and tag. Landing page with the games: <https://isene.org/funkey/> |
