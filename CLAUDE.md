...
| `fe2o3-funkey` | `funkey` | <https://github.com/isene/funkey> | Game engine for the terminal (battery rules waived for games). A pixel Frame shown as half-block cells, Input with a held state from key repeats, a fixed-step run loop, Sprite, Tilemap and Body physics. Planned: a sector/BSP renderer for Doom-style games, a software 3D rasterizer, pixel output through kitty graphics once glass has it. Games live in examples/ or their own repos |
