...
| `fe2o3-funkey` | `funkey` | <https://github.com/isene/funkey> | Game engine for the terminal (battery rules waived for games). A pixel Frame shown as half-block cells or as real pixels through kitty graphics, Input with exact key holds where the terminal reports releases, a fixed-step run loop, Sprite, Tilemap and Body physics, a WAD loader and a sector/BSP renderer. The examples are `climb` (a Jumpman-style platformer) and `doom` (the full game on any Doom WAD: monsters, weapons, pickups, specials, status bar, intermission, sound). Planned: a software 3D rasterizer. Games live in examples/ or their own repos |
